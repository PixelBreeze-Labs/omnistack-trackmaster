                        ...
                        <div>
                          ...
                          {submission.formData?.vehicleMakeModel && (
                            <div className="mt-2 text-xs">
                              ...
                              {(submission.formData.licensePlate || submission.formData.vehicleLicensePlate) && (
                                <div className="text-muted-foreground">
                                  Plate: {submission.formData.licensePlate || submission.formData.vehicleLicensePlate}
                                </div>
                              )}
                            </div>
                          )}
                      ...
                      {submission.attachmentUrls && submission.attachmentUrls.length > 0 ? (
                        <div className="space-y-2">
                          {submission.attachmentUrls.map((url, index) => {
                            ...
                            const fileInfo = submission.metadata?.fileInfo?.find(
                              (file) => file.url === url
                            );
                            const fileName = fileInfo?.name || `File ${index + 1}`;
                            const fileType = fileInfo?.type || "";
                            const isIdDocument = fileInfo?.isIdDocument;

                            return (
                              <div 
                                key={index} 
                                className="flex items-center gap-1 group cursor-pointer hover:bg-gray-50 rounded p-1"
