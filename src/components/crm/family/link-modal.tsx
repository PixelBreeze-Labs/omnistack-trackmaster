...
export function LinkFamilyModal({ isOpen, onClose, onSuccess, existingFamilyId }: LinkFamilyModalProps) {
  ...
  const handleAddMember = (customer: Customer, relationship: string) => {
    if (!existingFamilyId && customer._id === mainCustomer?._id) {
      toast.error("Can't add main customer as a member");
      return;
    }

    if (selectedMembers.some(m => m.id === customer._id)) {
                ...
                <ScrollArea className="h-[300px] rounded-md border">
                  {searchResults?.map((customer) => (
                    <div
                      ...
                      onClick={() => setMainCustomer(customer)}
                    >
                      <div className="flex items-center gap-3">
                        <Avatar>
                          <AvatarImage src={customer.avatar} />
            ...
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-medium">
                  {existingFamilyId ? 'Select Additional Members' : 'Select Family Members'}
                </h3>
                {!existingFamilyId && mainCustomer && (
              ...
              <ScrollArea className="h-[300px] rounded-md border">
                {searchResults?.map((customer) => (
                  <CustomerSelectCard
                    ...
                    isSelected={selectedMembers.some(m => m.id === customer._id)}
